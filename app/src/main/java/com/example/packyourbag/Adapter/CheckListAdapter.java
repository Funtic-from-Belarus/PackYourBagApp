...
public class CheckListAdapter extends RecyclerView.Adapter<CheckListViewHolder> {
    ...
    public CheckListAdapter(Context context, List<Items> itemsList, RoomDB roomDB, String show) {
        this.context = context;
        this.itemsList = itemsList;
        this.database = roomDB;
        this.show = show;
        if(itemsList.size() == 0){
            Toast.makeText(context.getApplicationContext(), "Никаких записей нет", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void onBindViewHolder(@NonNull CheckListViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        holder.checkBox.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                database.mainDao().checkUnckeck(itemsList.get(position).getID(), check);
                if(MyConstants.FALSE_STRING.equals(show)){
                    ...
                    if(itemsList.get(position).getChecked()){
                        tostMessage = Toast.makeText(context, "(" + holder.checkBox.getText() + ") упаковано", Toast.LENGTH_SHORT);
                    } else {
                        tostMessage = Toast.makeText(context, "(" + holder.checkBox.getText() + ") неупаковано", Toast.LENGTH_SHORT);
                    }
                    tostMessage.show();
                }
            }
        ...
        holder.btnDelete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                new AlertDialog.Builder(context)
                        .setTitle("Удалить ( "+ itemsList.get(position).getItemname()+" )")
                        .setMessage("Вы уверены?")
                        .setPositiveButton("Подтвердите", new DialogInterface.OnClickListener() {
                            @Override
                        ...
                        .setNegativeButton("Закрыть", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {
                                Toast.makeText(context, "Закрыто", Toast.LENGTH_SHORT).show();
                            }
                        })
